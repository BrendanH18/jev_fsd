...
export function sampleCandidates(snap, world) {
  ...
  if (!lost && !offRoad) {
    ...
    const speeds = new Set([0, Math.max(0, v - 3), v, Math.min(limit + 1, v + 2), limit].map((x) => Math.round(Math.max(0, Math.min(limit + 1, x)) * 10) / 10));
    for (const d of offsets) {
      for (const vt of [...speeds].sort((a, b) => b - a)) {
        const name = vt <= 0.05 ? "stop" : Math.abs(vt - v) < 0.3 ? "hold" : vt >= limit - 0.05 ? "limit" : vt < v ? "slow" : "faster";
        const prefix = d === 0 ? "keep_lane" : `${d < 0 ? "left" : "right"}_${Math.abs(d)}`;
        if (d !== 0 && name !== "hold" && name !== "slow") continue;  // lateral shifts only at hold/slow speeds
